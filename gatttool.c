...
static void events_handler(const uint8_t *pdu, uint16_t len, gpointer user_data)
{
    ...
    printf("%i",pdu[8]+pdu[9]*256); //added for debugging
	...
	g_print(" dist = ");
	g_print("%f4.2",distance_traveled);
	g_print(" speed = ");
	g_print("%f4.2",current_speed);
...
{   //added start
	...
	setup_abs(joystick_node, ABS_THROTTLE,  -32767, 32767); //commented out as it cause an operation error // was ABS_THRROTTTLE
...
//     (ioctl(fd           , UI_DEV_CREATE))	
    ...
    ev[0].type = EV_ABS;
    ev[0].code = ABS_THROTTLE; //was ABS_X;
    ev[0].value = -32767; 
    //ev[1].type = EV_SYN;
    //ev[1].code = SYN_REPORT;
    //ev[1].value = 0;
